...
def fetch_spacex_articles(api_key, conn):
    ...
    if last_searching_year is None:
        # If the table is empty, start from 2012
        pub_year = 2012
    elif last_searching_year[0] == 2012:
        pub_year = 2014
    else:
        pub_year = int(last_searching_year[0]) + 1

    for print_page in range(0, 30):
        params = {
            "q": "spacex stock increase",
            "fq": f'pub_year:({pub_year})',
            "page": print_page,
            "api-key": api_key
        }
        ...
        if response.status_code == 200:
            ...
            for article in articles:
                ...
                try:
                    # Get the ROWID of the last row
                    cursor.execute("SELECT ROWID FROM spacex_articles ORDER BY ROWID DESC LIMIT 1")
                    last_rowid = cursor.fetchone()

                    if last_rowid:
                        # Insert into the next line
                        ...
                        # If the table is empty, insert as the first row
                        cursor.execute("INSERT INTO spacex_articles (headline, formatted_date, searching_year) VALUES (?, ?, ?)",
                                       (headline, formatted_date, pub_year))
                        conn.commit()
